...
for ($i=0; $i < $how; $i++) {
	...
	echo "<h2 title=\"$myrow[book_title]\"><a href=\"/book.php/?id=$myrow[book_id]\">";
	echo "$myrow[book_title]</a></h2></div>";
	echo "<div class=\"img_bg\">";
	echo "<a href=\"/book.php/?id=$myrow[book_id]\"><img src=\"$myrow[book_img]\"></a></div>";
	echo "<div class=\"boto\">";
	//вытаскиваем в цикле авторов, если есть
	$select_sql_2 = "SELECT author FROM book_author WHERE book=$myrow[book_id]";
	...
	echo "<a href=\"/book.php/?id=$myrow[book_id]#order\"><button>$myrow[book_price] грн.</button></a>";
	echo "</div>";
	echo "</article>";
}
?>
